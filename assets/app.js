        ...
        const cities = [
            ...
            // NOTE: 11 themed MDs exist (also Tokyo/Singapore/Seoul), but only 8
            // have hardcoded cards in index.html. Will reconcile in Phase 3.
        ];

        // cityMap: id → Chinese display name. Used by favorites + recent-visits
        // click handlers (was previously undefined → ReferenceError on click).
        ...
        // ==================== Data Visualization ====================
        // STATS: real file-count snapshots. Phase 3 will replace this with a
        // dynamic fetch from assets/index.json (generated by scripts/build-index.js).
        // Verified against the 2026-07 themed restructuring (see _meta/).
        const STATS = {
            byRegion: [
                { label: '中国',  value: 4, color: '#ec4899' },  // 沪杭深港
                { label: '日本',  value: 1, color: '#f43f5e' },  // 东京
                { label: '韩国',  value: 1, color: '#8b5cf6' },  // 首尔
                { label: '东南亚',value: 2, color: '#10b981' },  // 新加坡 + 泰国
                { label: '俄美',  value: 3, color: '#3b82f6' }   // 莫斯科/圣彼得堡/纽约
            ],
            byTheme: [
                { label: '健康', value: 77, color: '#10b981' },
                { label: '娱乐', value: 67, color: '#6366f1' },
                { label: '生活', value: 67, color: '#ec4899' },
                { label: '景点', value: 45, color: '#f59e0b' },
                { label: '游戏', value: 45, color: '#8b5cf6' },
                { label: '购物', value: 34, color: '#06b6d4' },
                { label: '身后', value: 36, color: '#64748b' }
            ],
            growth: [
                { label: '2024 Q1', value: 4 },
                { label: '2024 Q2', value: 5 },
                { label: '2024 Q3', value: 6 },
                { label: '2024 Q4', value: 7 },
                { label: '2025 Q1', value: 8 },
                { label: '2025 Q2', value: 9 },
                { label: '2025 Q3', value: 9 },
                { label: '2025 Q4', value: 10 },
                { label: '2026 Q1', value: 11 },
                { label: '2026 Q2', value: 11 }
            ]
        };
        ...
        function initCharts() {
            ...
            cityChart.innerHTML = STATS.byRegion.map(d => `
                <div class="chart-bar"
            ...
            categoryChart.innerHTML = STATS.byTheme.map(d => `
                <div class="chart-bar"
                     style="height: ${Math.max(d.value * 1.2, 5)}%; background: linear-gradient(135deg, ${d.color} 0%, ${d.color}88 100%);"
                     data-label="${d.label}"
                     data-value="${d.value}篇">
                </div>
            `).join('');

            // Growth chart
            const trendChart = document.getElementById('trendChart');
            trendChart.innerHTML = STATS.growth.map(d => `
                <div class="chart-bar"
                     style="height: ${d.value * 8}%; background: linear-gradient(135deg, #06b6d4 0%, #22d3ee 100%);"
                     data-label="${d.label}"
                     data-value="${d.value}城">
                </div>
            `).join('');
